...
- 동적 처리 대상도 먼저 mock data로 분리한 뒤, API 연동 단계에서 실제 데이터로 교체한다.
...
2차 준비 단계에서 아래 값을 `mockTabletData.ts`로 분리한다.
...
3차 관리자/API 연동 단계에서 mock data를 실제 API 응답으로 교체한다.
...
- [ ] 공통 세션 정보 mock data 분리
- [ ] 학생/팀/출석 mock data 분리
- [ ] 활동 순서/진행률 mock data 분리
- [ ] 자료실 mock data 분리
- [ ] 교사용 모니터링 mock data 분리
- [ ] 화면 결과가 원본과 동일한지 확인
- [ ] 사용자 검수 OK

### 7단계. 관리자/API/DB 연동 준비

- [ ] 관리자에서 관리되어야 하는 데이터 항목 목록화
- [ ] 기존 관리자 메뉴/API 재사용 가능성 확인
- [ ] 신규 API 필요 여부 정리
- [ ] DB 테이블/컬럼/코드 변경 필요 여부 정리
- [ ] 인터페이스정의서, 테이블정의서, 프로그램정의서 갱신 대상 기록
- [ ] 사용자 확인 후 API/DB 작업 진행
