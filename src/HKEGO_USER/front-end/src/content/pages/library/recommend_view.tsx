export default function LibraryRecommendViewContent() {
	return (
		<>
			<section className="library_wrap inner" aria-labelledby="page-title">
				<div className="library_view_wrap">
					<div className="book_view_top">
						<div aria-hidden="true" className="imgfit">
							<img src="/pub/images/img_sample_library.webp" alt="" />
						</div>
						<div className="txt">
							<h1 id="page-title" className="tit">
								{"마음을 읽고 꿈을 쓰는 초등 사회정서 진로 워크북 1~2단계 나를 찾는 꿈"}
							</h1>
							<ul className="writer_info flex colm">
								<li>
									<strong>
										{"지은이"}
									</strong>
									{"홍길동"}
								</li>
								<li>
									<strong>
										{"출판사"}
									</strong>
									{"홍길동 출판사"}
								</li>
								<li>
									<strong>
										{"발행년도"}
									</strong>
									{"2026"}
								</li>
								<li>
									<strong>
										{"등록번호"}
									</strong>
									{"US000083252"}
								</li>
								<li>
									<strong>
										{"청구기호"}
									</strong>
									{"362.11-26-1"}
								</li>
								<li>
									<strong>
										{"도서위치"}
									</strong>
									{"도서관(2층)"}
								</li>
							</ul>
						</div>
					</div>
					<h2 className="tt">
						{"책소개"}
					</h2>
					<div className="book_view_about">
						{"\r\n\t\t\t책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다."}
						<br />
						{"\r\n\t\t\t책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다."}
						<br />
						{"\r\n\t\t\t책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다. 책 소개 내용입니다.\r\n\t\t"}
					</div>
				</div>
				<h2 className="stit large">
					{"관련 자료"}
				</h2>
				<ul className="flex book_list">
					<li>
						<a href="/library/search_view">
							<span aria-hidden="true" className="imgfit">
								<img src="/pub/images/img_sample_library.webp" alt="" />
								<span className="hover">
									<span>
										{"자세히 보기"}
									</span>
									<p>
										{"『빈센트 반 고흐, 세상을 노랗게 물들이다』는 아이들이 빈센트 반 고흐의 삶을 배울 수 있도록 눈높이를 맞춰 소개한 책이다. 살아생전 2000점이 넘는 그림을 그렸지만 단 한 점 밖에 팔리지 않았던 불운한 화가, 동서양을 통틀어 가장 사랑받는 화가인 고흐. 그런 고흐의 삶과 작품 속에 담긴 철학과 예술혼을 자연스럽게 보여준다."}
									</p>
									<ul className="writer_info flex colm">
										<li>
											<strong>
												{"출판사"}
											</strong>
											{"비룡소"}
										</li>
										<li>
											<strong>
												{"지은이"}
											</strong>
											{"유은실 소설"}
										</li>
									</ul>
								</span>
							</span>
							<p>
								{"빈센트 반 고흐, 세상을 노랗게 물들이다"}
							</p>
						</a>
					</li>
					<li>
						<a href="/library/search_view">
							<span aria-hidden="true" className="imgfit">
								<img src="/pub/images/img_sample_library.webp" alt="" />
								<span className="hover">
									<span>
										{"자세히 보기"}
									</span>
									<p>
										{"『빈센트 반 고흐, 세상을 노랗게 물들이다』는 아이들이 빈센트 반 고흐의 삶을 배울 수 있도록 눈높이를 맞춰 소개한 책이다. 살아생전 2000점이 넘는 그림을 그렸지만 단 한 점 밖에 팔리지 않았던 불운한 화가, 동서양을 통틀어 가장 사랑받는 화가인 고흐. 그런 고흐의 삶과 작품 속에 담긴 철학과 예술혼을 자연스럽게 보여준다."}
									</p>
									<ul className="writer_info flex colm">
										<li>
											<strong>
												{"출판사"}
											</strong>
											{"비룡소"}
										</li>
										<li>
											<strong>
												{"지은이"}
											</strong>
											{"유은실 소설"}
										</li>
									</ul>
								</span>
							</span>
							<p>
								{"빈센트 반 고흐, 세상을 노랗게 물들이다"}
							</p>
						</a>
					</li>
					<li>
						<a href="/library/search_view">
							<span aria-hidden="true" className="imgfit">
								<img src="/pub/images/img_sample_library.webp" alt="" />
								<span className="hover">
									<span>
										{"자세히 보기"}
									</span>
									<p>
										{"『빈센트 반 고흐, 세상을 노랗게 물들이다』는 아이들이 빈센트 반 고흐의 삶을 배울 수 있도록 눈높이를 맞춰 소개한 책이다. 살아생전 2000점이 넘는 그림을 그렸지만 단 한 점 밖에 팔리지 않았던 불운한 화가, 동서양을 통틀어 가장 사랑받는 화가인 고흐. 그런 고흐의 삶과 작품 속에 담긴 철학과 예술혼을 자연스럽게 보여준다."}
									</p>
									<ul className="writer_info flex colm">
										<li>
											<strong>
												{"출판사"}
											</strong>
											{"비룡소"}
										</li>
										<li>
											<strong>
												{"지은이"}
											</strong>
											{"유은실 소설"}
										</li>
									</ul>
								</span>
							</span>
							<p>
								{"빈센트 반 고흐, 세상을 노랗게 물들이다"}
							</p>
						</a>
					</li>
					<li>
						<a href="/library/search_view">
							<span aria-hidden="true" className="imgfit">
								<img src="/pub/images/img_sample_library.webp" alt="" />
								<span className="hover">
									<span>
										{"자세히 보기"}
									</span>
									<p>
										{"『빈센트 반 고흐, 세상을 노랗게 물들이다』는 아이들이 빈센트 반 고흐의 삶을 배울 수 있도록 눈높이를 맞춰 소개한 책이다. 살아생전 2000점이 넘는 그림을 그렸지만 단 한 점 밖에 팔리지 않았던 불운한 화가, 동서양을 통틀어 가장 사랑받는 화가인 고흐. 그런 고흐의 삶과 작품 속에 담긴 철학과 예술혼을 자연스럽게 보여준다."}
									</p>
									<ul className="writer_info flex colm">
										<li>
											<strong>
												{"출판사"}
											</strong>
											{"비룡소"}
										</li>
										<li>
											<strong>
												{"지은이"}
											</strong>
											{"유은실 소설"}
										</li>
									</ul>
								</span>
							</span>
							<p>
								{"빈센트 반 고흐, 세상을 노랗게 물들이다"}
							</p>
						</a>
					</li>
				</ul>
			</section>
		</>
	)
}
