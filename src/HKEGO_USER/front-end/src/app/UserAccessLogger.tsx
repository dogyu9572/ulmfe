...
async function recordAccess(pathname: string): Promise<void> {
	...
	if (!csrfToken) {
		const tokenResponse = await fetch(withBasePath('/api/user/main'), { credentials: 'include' })
		csrfToken = tokenResponse.headers.get(CSRF_HEADER_NAME) || readCsrfToken()
	}

	if (!csrfToken) return

	await fetch(withBasePath('/api/user/main'), {
		method: 'POST',
		credentials: 'include',
		headers: {
		...
		body: JSON.stringify({
			...
			accessType: pathname === '/' || pathname === '' ? 'MAIN' : 'PAGE'
		}),
		keepalive: true
	})
}
