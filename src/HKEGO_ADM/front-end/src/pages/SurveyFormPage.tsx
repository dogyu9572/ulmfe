...
export const SurveyFormPage: React.FC = () => {
	const [list, setList] = useState<SurveyForm[]>([])
	const [form, setForm] = useState<SurveyForm>(defaultForm)
	...
	const fetchList = useCallback(async (targetPage = page, targetSize = pageSize) => {
		...
		try {
			...
			if (!result.success || !result.data) {
				setError(result.message || '설문지 목록 조회에 실패했습니다.')
				return
			}
			...
			setError('설문지 목록 조회 중 오류가 발생했습니다.')
		}
	...
	const openEditPopup = async (qstnrSn: number | null) => {
		...
		setSelectedQuestionIndexes(new Set())
		try {
			const res = await fetch(`${BACKEND}/api/admin/survey-forms/${qstnrSn}`, { credentials: 'include' })
			const result: ApiResponse<SurveyForm> = await res.json()
			if (!result.success || !result.data) {
				setError(result.message || '설문지 상세 조회에 실패했습니다.')
				return
			}
			setForm({
				...defaultForm(),
			...
			setError('설문지 상세 조회 중 오류가 발생했습니다.')
		} finally {
			setLoading(false)
		}
	}
	...
	const saveForm = async () => {
		...
		try {
			...
			const res = await fetch(
				isEdit
					? `${BACKEND}/api/admin/survey-forms/${form.qstnrSn}`
					: `${BACKEND}/api/admin/survey-forms`,
				{
			...
			setMessage(isEdit ? '설문지가 수정되었습니다.' : '설문지가 등록되었습니다.')
			closePopup()
	...
	const deleteForm = async (row: SurveyForm) => {
		if (!row.qstnrSn) return
		if (!window.confirm(`"${row.qstnrNm}" 설문지를 삭제하시겠습니까?`)) return
		setLoading(true)
		setError(null)
		setMessage(null)
		try {
			const res = await fetch(`${BACKEND}/api/admin/survey-forms/${row.qstnrSn}`, {
				method: 'DELETE',
				credentials: 'include'
			})
	...
	const bulkDeleteForms = async () => {
		...
		if (targets.length === 0) {
			setError('삭제할 설문지를 선택하세요.')
			return
		}
		if (!window.confirm('선택한 설문지를 삭제하시겠습니까?')) return
		setLoading(true)
		setError(null)
		setMessage(null)
		try {
			for (const qstnrSn of targets) {
				const res = await fetch(`${BACKEND}/api/admin/survey-forms/${qstnrSn}`, {
					method: 'DELETE',
					credentials: 'include'
				})
				const result: ApiResponse<unknown> = await res.json()
	...
	const downloadResults = async (row: SurveyForm) => {
		if (!row.qstnrSn) return
		setLoading(true)
		setError(null)
		try {
			const res = await fetch(`${BACKEND}/api/admin/survey-forms/${row.qstnrSn}/results.xlsx`, { credentials: 'include' })
			if (!res.ok) throw new Error('결과 다운로드에 실패했습니다.')
			const blob = await res.blob()
			const url = window.URL.createObjectURL(blob)
			const link = document.createElement('a')
			link.href = url
			link.download = timestampedExcelFileName('설문지', row.qstnrNm)
	...
	return (
		<AdminLayout title="설문지 관리">
			<CrudPageCard title="설문지 관리" error={popupOpen ? null : error} message={message}>
				...
				<div className="bbs-post-filters search-section">
					<div className="bbs-post-filter-row">
						<select className="bbs-post-filter-select" value="qstnrNm" aria-label="검색 조건" disabled>
							<option value="qstnrNm">설문지 이름</option>
						</select>
						<input
							type="text"
							value={searchKeyword}
			...
			<LayerPopup
				open={popupOpen}
				title={popupMode === 'new' ? '설문지 등록' : '설문지 수정'}
				onClose={closePopup}
				wideDouble
				footer={
					<>
