		...
		<TeacherShell title="실시간 위치" info="QR 스캔 기반 공간별 학생 위치 현황">
			<div className="page_scroll">
				<div className="current_location_map">
					...
					<div className="box f2"><div className="mapbox"><img src={pubUrl("/pub/images/img_current_location_map01.webp")} alt="" aria-hidden="true" /><ul className="map_point"><li className="p1">디지털창작실</li><li className="p2">미디어실</li><li className="p3">아이디어실 3, 4, 5</li><li className="p4">조리체험실</li><li className="p5">목공실</li><li className="p6">공작실 1, 2</li><li className="p7">아이디어실 1, 2</li></ul><ul className="team_point">{teamSummaries.filter((team) => locationPointClass(team.place) !== 'p1' && locationPointClass(team.place) !== 'p2' && locationPointClass(team.place) !== 'p3' && locationPointClass(team.place) !== 'p4').map((team) => <li className={`${team.className} ${locationPointClass(team.place)}`} key={team.teamName}><span><h3>{team.teamName}</h3><strong>{team.students.length}명</strong></span></li>)}</ul></div><p>본관 2층 : ESD배움터</p></div>
					<div className="box f1"><div className="mapbox"><img src={pubUrl("/pub/images/img_current_location_map02.webp")} alt="" aria-hidden="true" /><ul className="map_point"><li className="p1">사회존</li><li className="p2">미래존</li><li className="p3">도입존</li><li className="p4">지구존</li><li className="p5">ESD놀이터</li></ul><ul className="team_point">{teamSummaries.filter((team) => ['p1', 'p2', 'p3', 'p4'].includes(locationPointClass(team.place))).map((team) => <li className={`${team.className} ${locationPointClass(team.place)}`} key={team.teamName}><span><h3>{team.teamName}</h3><strong>{team.students.length}명</strong></span></li>)}</ul></div><p>본관 1층 : ESD체험터</p></div>
				</div>
				<div className="flex_half">
					...
					<div className="box"><h2 className="stit mt0">장소별 활동 참고</h2><ul className="location_info">{places.map((place) => <li key={place}><div className="tit">{place}</div><ul className="dots_list"><li>등록된 활동 장소</li></ul></li>)}{loaded && places.length === 0 && <li><div className="tit">장소 정보 없음</div><ul className="dots_list"><li>등록된 활동 장소가 없습니다.</li></ul></li>}</ul></div>
				</div>
			</div>
		</TeacherShell>
	)
