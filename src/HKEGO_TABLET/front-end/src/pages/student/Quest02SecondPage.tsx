...
export const Quest02SecondPage = () => {
	...
	const { content, questions, savedAnswers, title, location } = useQuestDynamicPage(1, 1)
	const { pageRef, submit, saving, restoreSavedAnswers } = useQuestSubmit({ routeIndex: 1, nextPath: '/student/quest02_3' })
	useEffect(() => restoreSavedAnswers(savedAnswers), [restoreSavedAnswers, savedAnswers])

	return (
		<main className="container" id="mainContent">
			<h1 className="sound_only">{title || '퀘스트2'}</h1>
			<StudentCaseHeader />
			<section className="basic_board">
				<QuestPageShell title={title} step="STEP 2 사건탐색 - 퀘스트2" location={location} pageRef={pageRef}>
