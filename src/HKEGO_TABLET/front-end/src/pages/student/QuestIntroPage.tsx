...
export const QuestIntroPage = () => {
	...
	const videoClasses = ['c1 video_end', 'c2', 'c3']
						...
						{videos.length > 0 ? videos.slice(0, 3).map((video, index) => (
							<li className={videoClasses[index] || `c${index + 1}`} key={`${video.contentName}-${index}`}>
								<a href={video.videoUrl || '/student/quest_video'} onClick={(event) => {
									...
									<div className="img" aria-hidden="true"><img src={`/pub/images/img_start_video0${Math.min(index + 1, 3)}.webp`} alt="" /></div>
									<div className="txt">
										...
										<div className="line_area">
											<div className="pct"><strong>{index === 0 ? 100 : 0}</strong>%</div>
											<div className="bar" style={{ width: `${index === 0 ? 100 : 0}%` }}><div className="pct" aria-hidden="true"><strong>0</strong>%</div></div>
										</div>
									</div>
								</a>
							</li>
